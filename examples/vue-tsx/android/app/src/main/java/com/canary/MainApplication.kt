...
// hand-maintained. A sensor whose `Module` class compiles into the app but is missing from this
// map fails at runtime, not build time: `Cannot find native module 'ExpoBarometer'`. Each key
// string must match that sensor's own `definition() { Name("...") }` exactly.
