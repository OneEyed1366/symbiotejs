...
// There's no `expo` aggregator project here to auto-generate a native-module name map, so this
// is hand-maintained — see the symbiote-expo-native-module skill / expo-native-module-setup
// howto. Each key string must match that sensor's own `definition() { Name("...") }` exactly;
// a sensor missing from this map compiles fine (the whole expo-sensors android/ project is
// linked) but fails at RUNTIME with `Cannot find native module 'ExpoX'`, no compile warning.
