...
export function requestFrame(callback: () => void): number {
  ...
  const run: RequestFrame = (cb) => Number(Reflect.apply(raf, globalThis, [cb]))
  return run(callback)
}
...
export function setTimer(callback: () => void, delayMs: number): TimerHandle {
...
export function clearTimer(handle: TimerHandle): void {
