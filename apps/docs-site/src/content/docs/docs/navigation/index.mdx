...
```sh
pnpm add @symbiote-native/navigation
```

`@symbiote-native/navigation` itself is a workspace package (`packages/navigation`), not yet
published — add it as a workspace dependency the same way the examples do.
