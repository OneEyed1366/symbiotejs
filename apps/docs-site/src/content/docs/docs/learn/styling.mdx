...
A build-time-only compiler (`@symbiote/css-parser`) turns each rule into a
...
`@symbiote/engine`). This registry is shared by every adapter, not just Vue —
...
import { Text, View } from '@symbiote/vue';
</script>
...
import { Text, View } from '@symbiote/angular';
import styles from './card.module.css';
...
`@symbiote/css-parser` ships two more pieces that close that gap for a
...
- **`@symbiote/css-parser/typescript-plugin`** is a TypeScript language
...
`@symbiote/engine` by every adapter. It's still fully supported — reach for
...
```vue
<script setup lang="ts">
import { StyleSheet, Text, View } from '@symbiote/vue';
