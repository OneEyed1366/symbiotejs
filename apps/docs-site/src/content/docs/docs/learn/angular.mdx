---
title: Angular guide
description: How to write Symbiote apps with the Angular adapter.
---
...
<Aside type="caution" title="One deliberate exception">
  ...
  packages have followed: [Slider](/symbiote/docs/packages/slider/) ships an Angular
  build too — see the [status page](/symbiote/docs/project/status/) for the exact
  line.
</Aside>
...
<img src={`${import.meta.env.BASE_URL}/angular-demo.gif`} alt="Angular driving a native symbiote demo" width="260" />
...
import { Pressable, Text, View } from '@symbiotejs/angular';
import './app.css';
...
inline instead of a CSS file — see the [Styling guide](/symbiote/docs/learn/styling/)
for both paths.
...
- every symbiote component is `standalone: true` — import it directly in the
...
- imperative handles use `@ViewChild(..., { read: ElementRef })` — the
  `nativeElement` is the symbiote host node, not a DOM element;
...
Read the exact API surface in the [Angular API reference](/symbiote/docs/api/angular/).
