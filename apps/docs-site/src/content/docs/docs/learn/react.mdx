...
import { Pressable, Text, View } from '@symbiote/react';
...
- imperative APIs use React refs, for example `ref.current?.focus()` on
  `TextInput`;
- app code imports from `@symbiote/react`, while the consuming app still pins
  `react` and `react-native` at the top level.

## What Symbiote owns

React still owns hooks, component state, and render scheduling. The adapter owns
only the host bridge: it maps React reconciler mutations to `@symbiote/engine`.
