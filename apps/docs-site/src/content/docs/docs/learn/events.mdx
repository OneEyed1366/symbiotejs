...
<Aside type="caution" title="Scroll events stay callback inputs">
  ...
  value. See the [Angular API reference](/symbiote/docs/api/angular/) for the exact
  split.
</Aside>
...
Symbiote commands native back to the JavaScript value.

That correction logic is shared in `@symbiotejs/components`; React, Vue, and
