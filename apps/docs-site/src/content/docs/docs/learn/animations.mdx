...
`loop`, `delay`) live once in `@symbiote/engine` — every adapter re-exports the
...
```tsx
import { Animated } from '@symbiote/react';
...
import { Animated } from '@symbiote/vue';
...
import { Animated, AnimatedView } from '@symbiote/angular';
...
<Aside type="caution" title="Two different reasons for one workaround">
  ...
  a direct named import binding. `@symbiote/angular` exports `AnimatedView`,
