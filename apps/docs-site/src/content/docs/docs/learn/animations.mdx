...
Symbiote ships a from-scratch, framework-agnostic port of React Native's
...
`loop`, `delay`) live once in `@symbiotejs/engine` — every adapter re-exports the
...
```tsx
import { Animated } from '@symbiotejs/react';
...
import { Animated } from '@symbiotejs/vue';
...
import { Animated, AnimatedView } from '@symbiotejs/angular';
...
<Aside type="caution" title="Two different reasons for one workaround">
  ...
  a direct named import binding. `@symbiotejs/angular` exports `AnimatedView`,
...
[`examples/react/App.tsx`](https://github.com/OneEyed1366/symbiote/blob/master/examples/react/App.tsx)'s
...
Symbiote's `Animated` is the RN `Animated` API, not Reanimated — there is no
...
itself is not tested against symbiote's engine; it hooks much deeper into RN's
runtime than a plain native-view library does, so treat it as unverified
rather than assuming it works the same way a
[wrapped third-party native view](/symbiote/docs/howtos/third-party-views/) would.
