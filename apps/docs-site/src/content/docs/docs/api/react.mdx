...
Import app-facing APIs from `@symbiote/react`:

```tsx
import { Pressable, ScrollView, StyleSheet, Text, TextInput, View } from '@symbiote/react';
```

The React adapter intentionally feels close to React Native. The difference is
below the component surface: host mutations go through `@symbiote/engine`.
...
import { TextInput, type ITextInputHandle } from '@symbiote/react';
...
```tsx
import { Alert, Dimensions, Platform, StyleSheet } from '@symbiote/react';
```
...
from `@symbiote/react` — see the [Animations guide](/symbiote/docs/learn/animations/)
for the full surface and per-driver tradeoffs.

## Portals

```tsx
import { createPortal } from '@symbiote/react';
...
container operations a portal needs. `@symbiote/react` is mutation-mode, so
...
```tsx
import { createTunnel } from '@symbiote/react';
...
`@symbiote/react` exports the same `AppRegistry`/`setHostRegistrar` entry point
...
```tsx
import { AppRegistry } from '@symbiote/react';
