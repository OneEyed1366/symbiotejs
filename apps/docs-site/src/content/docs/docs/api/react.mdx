...
| Styles          | `style={styles.root}` with React Native-style objects, or `className` for a registered CSS class (see the [Styling guide](/symbiote/docs/learn/styling/)) |
...
from `@symbiotejs/react` — see the [Animations guide](/symbiote/docs/learn/animations/)
...
The target (`containerNodeOrRef`) must be an already-mounted symbiote host
...
[Vue API reference](/symbiote/docs/api/vue/#portals-teleport).
...
described in the [Core API](/symbiote/docs/api/core/#app-entry-point-appregistry). A
