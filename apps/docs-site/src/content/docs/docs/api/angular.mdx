...
Import app-facing APIs from `@symbiotejs/angular`:

```ts
import { Pressable, ScrollView, StyleSheet, Text, TextInput, View } from '@symbiotejs/angular';
```

Every symbiote component is a `standalone: true` Angular component — add it to
...
| Refs            | `@ViewChild('host', { read: ElementRef })`, `.nativeElement` is the symbiote host node |
...
`@symbiotejs/components` state machines already define (`IPressHandler`,
...
import { TextInput, type ITextInputHandle } from '@symbiotejs/angular';
...
import { PortalDirective, PortalOutletDirective, Text, View } from '@symbiotejs/angular';
...
```ts
import { Component, signal } from '@angular/core';
import { createTunnel, Text, TunnelInDirective, TunnelOut, View } from '@symbiotejs/angular';
...
`@symbiotejs/angular` exports the same `AppRegistry`/`setHostRegistrar` entry
point described in the
[Core API](/symbiote/docs/api/core/#app-entry-point-appregistry). One Angular-specific
...
```ts
import { AppRegistry } from '@symbiotejs/angular';
...
```ts
import { Alert, Dimensions, Platform, StyleSheet } from '@symbiotejs/angular';
```
...
import { ColorSchemeService, WindowDimensionsService } from '@symbiotejs/angular';
...
re-exported from `@symbiotejs/angular` — see the
[Animations guide](/symbiote/docs/learn/animations/) for the full surface and why the
named exports exist.
...
the whole tree isn't a symbiote quirk, it's why demo screens are split into
...
instead — see the [Slider package](/symbiote/docs/packages/slider/) for the reference
