...
Import app-facing APIs from `@symbiote/angular`:

```ts
import { Pressable, ScrollView, StyleSheet, Text, TextInput, View } from '@symbiote/angular';
```
...
`@symbiote/components` state machines already define (`IPressHandler`,
...
import { TextInput, type ITextInputHandle } from '@symbiote/angular';
...
import { PortalDirective, PortalOutletDirective, Text, View } from '@symbiote/angular';
...
```ts
import { Component, signal } from '@angular/core';
import { createTunnel, Text, TunnelInDirective, TunnelOut, View } from '@symbiote/angular';
...
`@symbiote/angular` exports the same `AppRegistry`/`setHostRegistrar` entry
...
```ts
import { AppRegistry } from '@symbiote/angular';
...
```ts
import { Alert, Dimensions, Platform, StyleSheet } from '@symbiote/angular';
```
...
import { ColorSchemeService, WindowDimensionsService } from '@symbiote/angular';
...
re-exported from `@symbiote/angular` — see the
