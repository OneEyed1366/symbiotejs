...
Most app code should import from `@symbiote/react` or `@symbiote/vue`. The core
...
```ts
import { AppRegistry, setHostRegistrar } from '@symbiote/react'; // or '@symbiote/vue'
...
`@symbiote/engine`'s `createAppRegistry` — the only framework-specific piece
