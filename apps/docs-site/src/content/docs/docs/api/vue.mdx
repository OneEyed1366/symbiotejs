...
Import app-facing APIs from `@symbiote/vue`:

```ts
import { Pressable, ScrollView, StyleSheet, Text, TextInput, View } from '@symbiote/vue';
```
...
import { ref } from 'vue';
import { TextInput, type ITextInputHandle } from '@symbiote/vue';
...
`@symbiote/vue`) are the shared helpers behind this, for wrapper authors adding
...
`@symbiote/vue` re-exports the same runtime utilities as the other adapters,
...
from `@symbiote/vue` — see the [Animations guide](/symbiote/docs/learn/animations/)
...
import { View, type ISymbioteNode } from '@symbiote/vue';
...
`@symbiote/vue` exports the same `AppRegistry`/`setHostRegistrar` entry point
...
```ts
import { AppRegistry } from '@symbiote/vue';
