...
- Styles: `:style="styles.root"` with React Native-style objects. Most
  ...
  the [Styling guide](/symbiote/docs/learn/styling/).
...
`Switch`, `TextInput`, and the [Slider](/symbiote/docs/packages/slider/) wrapper accept
...
`v-show` works on any symbiote host node without extra setup — it toggles the
...
from `@symbiotejs/vue` — see the [Animations guide](/symbiote/docs/learn/animations/)
...
described in the [Core API](/symbiote/docs/api/core/#app-entry-point-appregistry). One
