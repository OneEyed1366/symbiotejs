...
`@symbiote/slider` wraps the `RNCSlider` native view from
...
`@symbiote/slider` itself is a workspace package (`packages/slider`), not yet published — add it
...
<Aside type="caution">
  ...
  `@symbiote/slider/react` or `@symbiote/slider/vue`, never from the community package itself.
</Aside>
...
`@symbiote/slider` ships **zero runtime metadata** for `RNCSlider` — the engine derives its
