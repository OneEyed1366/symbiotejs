...
`@symbiotejs/css-parser` compiles the rule at build time; `className="card"`
...
Every adapter re-exports `StyleSheet` from `@symbiotejs/engine`. It's still
...
you wire two pieces from `@symbiotejs/css-parser` — add it as a devDependency:
...
[Styling guide](/symbiotejs/docs/learn/styling/#real-key-narrowing-css-dts-and-the-typescript-plugin)
...
hatch, CSS Modules status), see the [Styling guide](/symbiotejs/docs/learn/styling/).
