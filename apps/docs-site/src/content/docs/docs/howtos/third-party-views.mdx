...
## Reference implementation: `@symbiote/slider`
...
The consuming app installs only `@symbiote/slider` — the wrapper package
...
must only ever list `@symbiote/<lib>` — never the third-party native package
...
```json
"@symbiote/slider": {
  "root": ".../examples/react/node_modules/@symbiote/slider",
  "platforms": {
    "ios": {
      "podspecPath": ".../node_modules/@symbiote/slider/symbiote-slider.podspec"
    },
    "android": {
...
from the app's single `@symbiote/<lib>` dependency, with the third-party
