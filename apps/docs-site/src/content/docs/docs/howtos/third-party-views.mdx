...
native view through the same `createNode`-by-ViewConfig path Symbiote uses
...
everywhere else in Symbiote.

## Reference implementation: `@symbiotejs/slider`
...
The consuming app installs only `@symbiotejs/slider` — the wrapper package
...
must only ever list `@symbiotejs/<lib>` — never the third-party native package
...
```json
"@symbiotejs/slider": {
  "root": ".../examples/react/node_modules/@symbiotejs/slider",
  "platforms": {
    "ios": {
      "podspecPath": ".../node_modules/@symbiotejs/slider/symbiote-slider.podspec"
    },
    "android": {
...
from the app's single `@symbiotejs/<lib>` dependency, with the third-party
...
the project's
[`symbiote-third-party-native-view`](https://github.com/OneEyed1366/symbiote/blob/master/.claude/skills/symbiote-third-party-native-view/SKILL.md)
skill.
