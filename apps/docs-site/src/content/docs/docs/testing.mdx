...
An app built with symbiote is tested the same way as any other React Native
app — symbiote doesn't ask for a different tool. Two layers: headless
...
<Aside type="tip" title="One spec file, shared across React and Vue">
  Detox attaches to the stock React Native host **below** the layer symbiote
