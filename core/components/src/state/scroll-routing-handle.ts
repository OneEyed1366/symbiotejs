...
// (VirtualizedList, VirtualizedSectionList, and — via those — FlatList/SectionList).
// Each method forwards straight to the underlying ScrollView's own handle/node; neither
// list adds behavior of its own, so the 6 members live here ONCE and both list handle
// types extend this instead of hand-duplicating the signatures. Extracted so the two
// HANDLE TYPES cannot drift from each other the way independently-declared copies could.
