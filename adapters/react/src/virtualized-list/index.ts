...
export interface IVirtualizedListHandle {
  ...
  // ScrollView handle (or null before it attaches), with no fabricated native tag.
...
function isCellViewable(percent: number, config: IViewabilityConfig): boolean {
...
export function VirtualizedList<ItemT>(
  ...
  useEffect(() => {
    ...
    if (isWithinEndThreshold && lastCellRendered && sentEndForContentLengthRef.current !== total) {
  ...
  if (count === FIRST_INDEX) {
    ...
    for (let index = first; index <= last; index += 1) {
      ...
      if (separator !== undefined) {
        children.push(createElement('symbiote-view', { key: `sep-${key}` }, separator));
      }
    }
