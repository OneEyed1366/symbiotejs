...
function animatedNodesInTransforms(transforms: readonly TransformEntry[]): AnimatedNode[] {
...
export class AnimatedTransform extends AnimatedWithChildren {
  ...
  private readonly transforms: readonly TransformEntry[]
  ...
  constructor(nodes: readonly AnimatedNode[], transforms: readonly TransformEntry[]) {
    super()
    this.nodes = nodes
  ...
  override __getValue(): TransformEntry[] {
  ...
  override __getAnimatedValue(): TransformEntry[] {
    return this.transforms.map((entry) => {
      const result: TransformEntry = {}
      for (const key of Object.keys(entry)) {
        const value = entry[key]
